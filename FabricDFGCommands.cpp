...
SICALLBACK FabricCanvasOpApply_Execute(CRef &in_ctxt)
{
  ...
  do
  {
    ...
    // remove all L"_CanvasOut_*" custom parmameter sets.
    CRefArray propRefs = obj.GetLocalProperties();
    CString propString = L"";
    for (LONG i=0;i<propRefs.GetCount();i++)
    {
      if (propRefs[i].GetAsText().FindString(L"._CanvasOut_") == UINT_MAX)
        continue;
      if (!propString.IsEmpty())
        propString += L",";
      propString += propRefs[i].GetAsText();
    }
    if (!propString.IsEmpty())
    {
      CValueArray args;
    ...
    for (int i=0;i<portmap.size();i++)
    {
          ...
          && dfgTools::GetSiClassIdFromResolvedDataType(pmap.dfgPortDataType) == siParameterID)
      {
        ...
        CustomProperty prop;
        if (obj.AddCustomProperty(L"_CanvasOut_" + pmap.dfgPortName, false, prop) != CStatus::OK)
        {
        ...
        {
          success = (prop.AddParameter( L"value", CValue::siBool,
        ...
        {
          success = (prop.AddParameter( L"value", CValue::siDouble,
                                        siPersistable | siAnimatable | siKeyable | siReadOnly,
                                        L"", L"",
                                        CValue(), CValue(), CValue(), CValue(), CValue(),
                                        param) == CStatus::OK);
        ...
        {
          success = (prop.AddParameter( L"value", CValue::siInt4,
                                        siPersistable | siAnimatable | siKeyable | siReadOnly,
                                        L"", L"",
                                        CValue(), CValue(), CValue(), CValue(), CValue(),
                                        param) == CStatus::OK);
...
SICALLBACK FabricCanvasOpConnectPort_Init(CRef &in_ctxt)
{
  ...
  oArgs.Add(L"targetNames",             CString());   // semicolon separated target names. if empty then disconnect everything from the port.
...
SICALLBACK FabricCanvasOpConnectPort_Execute(CRef &in_ctxt)
{
  ...
  // case 2: connect targetNamesRaw to port.
  else
  {
    ...
    {
      // get target name.
      CString targetName = targetNames[i];
...
// ---
// command "FabricCanvasOpDisconnectPort".
// ---

SICALLBACK FabricCanvasOpDisconnectPort_Init(CRef &in_ctxt)
{
  Context ctxt(in_ctxt);
  Command oCmd;

  oCmd = ctxt.GetSource();
  oCmd.PutDescription(L"disconnects things from a port of a Canvas operator.");
  oCmd.SetFlag(siNoLogging, false);
  oCmd.EnableReturnValue(true);   // returns 'true' on success.
  ...
  oArgs.Add(L"OperatorName",            CString());
  oArgs.Add(L"portName",                CString());
  oArgs.Add(L"targetNames",             CString());   // if empty then disconnect everything from the port.

  return CStatus::OK;
}

SICALLBACK FabricCanvasOpDisconnectPort_Execute(CRef &in_ctxt)
{
  // init.
  Context ctxt(in_ctxt);
  ctxt.PutAttribute(L"ReturnValue", false); // init return value.
  ...
  else
  {
    ...
    for (LONG i=0;i<targetNames.GetCount();i++)
    {
      ...
      {
        LONG instance;
        if (!dfgTools::DisconnectFromPortGroup(op, portName, targetRef))
...
// ---
// command "FabricCanvasGetBindingID".
// ---

SICALLBACK FabricCanvasGetBindingID_Init(CRef &in_ctxt)
{
  Context ctxt(in_ctxt);
  Command oCmd;

  oCmd = ctxt.GetSource();
  oCmd.PutDescription(L"returns the ID of the FabricCore DFGBinding used by a specified Canvas operator.");
  oCmd.SetFlag(siNoLogging, false);
  oCmd.EnableReturnValue(true) ;
...
SICALLBACK FabricCanvasGetBindingID_Execute(CRef &in_ctxt)
{
  // init.
  Context ctxt(in_ctxt);
  CValueArray args = ctxt.GetAttribute(L"Arguments");
